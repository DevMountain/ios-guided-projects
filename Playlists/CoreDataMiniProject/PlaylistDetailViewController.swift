...
extension PlaylistDetailViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        // TEACHING NOTE: - songs are stored as an NSSet, and need to be cast as an array as shown below
        if let songSet = playlist?.songs,
            songs = Array(songSet) as? [Song] {
    ...
    func tableView(tableView: UITableView, commitEditingStyle editingStyle: UITableViewCellEditingStyle, forRowAtIndexPath indexPath: NSIndexPath) {
        if let songSet = playlist?.songs,
            songs = Array(songSet) as? [Song] where editingStyle == .Delete {
            let song = songs[indexPath.row]
            
            PlaylistController.sharedController.removeSongFromPlaylist(song)
